...
public class AddBookMenu {
	...
	public void menu(Scanner scanInput) throws LibraryServiceException {
		...
		Book book = new Book(random.nextLong(), title, price, vol, publishDate, matchedSubject);
	...
	public Subject chooseSubjectAssociation(Scanner scanInput) throws LibraryServiceException {
		...
		while (matchedSubject == null) {
			...
			System.out.println("\nPlease choose a subject title to associate this book to:");
			scanInput.nextLine();
			...
			if (match.isPresent()) {
				...
				System.out.println("\nSubject retrieved :" + matchedSubject);
			} else {
				System.out.println("\nNo Subject found please enter a valid name.");
			}
		}
